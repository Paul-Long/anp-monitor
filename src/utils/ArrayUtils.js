...
function countsort(
  ...
  if (sortByte == 3) {
    for (let i = 129; i < 256; i++) index[i] = index[i - 1] + count[i - 1];
    index[0] = index[255] + count[255];
    for (let i = 1; i < 128; i++) index[i] = index[i - 1] + count[i - 1];
  } else {
    for (let i = 1; i < 256; i++) index[i] = index[i - 1] + count[i - 1];
  }
...
function countsortDesc(
  ...
  if (sortByte == 7) {
    for (let i = 126; i >= 0; i--) index[i] = index[i + 1] + count[i + 1];
    index[255] = index[0] + count[0];
    for (let i = 254; i >= 128; i--) index[i] = index[i + 1] + count[i + 1];
  } else {
    for (let i = 254; i >= 0; i--) index[i] = index[i + 1] + count[i + 1];
  }
...
function radixsortDesc(keys, values, tempKeys, tempValues, offset, length) {
  ...
  if (tempValues == null) tempValues = new Array(length).fill(0);
...
function split(keys, values, left, right) {
  ...
  if (keys[left] > keys[middle]) swap(keys, values, left, middle);
  if (keys[middle] > keys[right]) swap(keys, values, middle, right);
  if (keys[left] > keys[middle]) swap(keys, values, left, middle);
...
function hybridsort(keys, values, left, right) {
  ...
  while (right - left >= 1) {
    if (right - left <= USE_RADIX) {
      ...
      let i = useMedian
        ? split(keys, values, left, right)
        : splitIndex(keys, values, left, right, randomizedIndex(left, right));
...
function hybridsortDesc(keys, values, tmpKeys, tmpValues, left, right) {
  ...
  while (right - left >= 1) {
    if (right - left <= USE_RADIX) {
      ...
      let i = useMedian
        ? splitDesc(keys, values, left, right)
        : splitDescIndex(
            keys,
            values,
            left,
            right,
            randomizedIndex(left, right)
          );
