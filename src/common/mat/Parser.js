...
const GCRootType = {
  ...
  '(Internal string)': {
    desc: 'kInternalizedString',
    value: GCRootInfo.JS_SYSTEM_OBJ
  },
  ...
  '(Global handles)': {
    desc: 'kGlobalHandles',
    value: GCRootInfo.JS_BINDING_GLOBAL
  },
  '(Eternal handles)': {
    desc: 'kEternalHandles',
    value: GCRootInfo.JS_BINDING_GLOBAL
  },
...
class Parser {
  constructor(profile, options) {
    ...
    this.nodeTraceNodeIdOffset = snapshot.meta.node_fields.indexOf(
      'trace_node_id'
    );
    // 获取 edge 描述 offset
    this.edgeTypeOffset = snapshot.meta.edge_fields.indexOf('type');
    this.edgeNameOrIndexOffset = snapshot.meta.edge_fields.indexOf(
      'name_or_index'
    );
  ...
  getFirstEdgeIndexs() {
    ...
    for (
      ...
      nodeOrdinal < this.nodeCount;
      ++nodeOrdinal
    ) {
      firstEdgeIndexes[nodeOrdinal] = edgeIndex;
      const offset =
        this.nodes[
          nodeOrdinal * this.nodeFieldLength + this.nodeEdgeCountOffset
        ] * this.edgeFiledLength;
      for (
        let i = edgeIndex;
        i < edgeIndex + offset;
        i += this.edgeFiledLength
      ) {
  ...
  *initAsync(cb) {
    ...
    yield cb({
      prefix: '显式 Maps 解析完毕',
      suffix: '准备开始解析隐式 Maps...'
    });
    this.reportInstances();
    yield cb({
      prefix: '隐式 Maps 解析完毕',
      suffix: '准备开始读取 Real Nodes...'
    });
    // 2.
    this.readNodes();
    yield cb({
      prefix: 'Real Nodes 读取完毕',
      suffix: '准备开始将 Ordinal ID 转换为 Real ID...'
    });
    // 3. 所有的 ordinal id -> real id
    this.map2ids();
    yield cb({
      prefix: 'Real ID 转换完毕',
      suffix: '准备开始计算 Dominator Tree...'
    });
  }
  ...
  readRoots() {
    ...
    for (let edge of strongRootsEdges) {
      ...
      const value =
        (GCRootType[nodeName] && GCRootType[nodeName].value) ||
        GCRootType[''].value;
      if (value !== -1) {
  ...
  reportInstances() {
    ...
    let sharedFunctionInfoMapAddress = this.namedRoots.get(
      'shared_function_info_map'
    );
    let cellMapAddress = this.namedRoots.get('cell_map');
    let globalPropertyCellMapAddress = this.namedRoots.get(
  ...
  readNodes() {
    ...
    for (
      ...
      if (
        !this.hasMap[nodeOrdinalId] &&
        this.implicitMap[nodeOrdinalId] === 0
      ) {
        const nodeType = this.nodeUtil.getType(nodeOrdinalId);
        if (
          nodeType === 'synthetic' ||
          this.indirectSize[nodeOrdinalId] !== -1
        ) {
          continue;
        }
      ...
      this.outboundIndexList[objectId] = this.readEdges(
        nodeOrdinalId,
        objectId
      );
  ...
  calculateStatics(nodeOrdinalId) {
    ...
    if (nodeType === 'array' || nodeName === 'Array') {
    ...
    switch (nodeType) {
      ...
      case 'sliced string':
      case 'string':
        this.statistics.string += nodeSelfSize;
        break;
      default:
        if (
          nodeType &&
          !Object.prototype.hasOwnProperty.call(this.statistics, nodeType)
        ) {
          this.statistics[nodeType] = 0;
        }
        if (nodeType) {
          this.statistics[nodeType] += nodeSelfSize;
        }
        break;
    }
  }
