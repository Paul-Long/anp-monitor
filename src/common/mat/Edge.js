...
class Edge {
  ...
  getType(id, source) {
    ...
    return types[
      this.parser.edges[sourceEdgeIndex + this.parser.edgeTypeOffset]
    ];
  }
  ...
  getTargetNode(id, source) {
    ...
    const sourceEdgeIndex = (source && id) || id * this.parser.edgeFiledLength;
    const targetNodeSourceIndex = this.parser.edges[
      sourceEdgeIndex + this.parser.edgeToNodeOffset
    ];
  ...
  getNameOrIndex(id, source) {
    ...
    const edgeNameOrIndex = this.parser.edges[
      sourceEdgeIndex + this.parser.edgeNameOrIndexOffset
    ];
    const name_or_index =
      edgeType === HeapGraphEdgeType.kElement ||
      edgeType === HeapGraphEdgeType.kHidden
        ? `[${edgeNameOrIndex}]`
        : `${this.parser.strings[edgeNameOrIndex]}`;
