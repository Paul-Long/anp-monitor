...
class Node {
  ...
  getType(id, source) {
    ...
    return types[
      this.parser.nodes[nodeSourceIndex + this.parser.nodeTypeOffset]
    ];
  }
  ...
  getName(id, source) {
    ...
    return strings[
      this.parser.nodes[nodeSourceIndex + this.parser.nodeNameOffset]
    ];
  }
  ...
  getEdges(id, source) {
    ...
    const nextFirstEdgeIndex =
      this.parser.firstEdgeIndexs[nodeOrdinalIndex + 1] ||
      this.parser.edges.length;
    const edges = [];
    for (
      let i = firstEdgeIndex;
      i < nextFirstEdgeIndex;
      i += this.parser.edgeFiledLength
    ) {
      edges.push(i);
    }
    return edges;
